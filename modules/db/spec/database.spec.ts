...
describe('database functionality', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [DB.DBModule.provideDB(todoAppSchema)],
    });
  });

  beforeEach(
    inject([DB.Database], (_idb: DB.Database) => {
      idb = _idb;
    })
  );
  ...
  it('should insert some data', done => {
    idb
      .insert('todos', [{ name: 'todo1' }, { name: 'todo2' }])
      .toArray()
      .subscribe(
        (results: any) => {
  ...
  it('should insert some more data', done => {
    idb
      .insert('todos', [{ name: 'todo3' }, { name: 'todo4' }])
      .toArray()
      .subscribe(
        (results: any) => {
  ...
  it('should insert some more data with a primary key', done => {
    idb
      .insert('users', [{ userID: 'user1' }, { userID: 'user2' }])
      .toArray()
      .subscribe(
        (results: any) => {
  ...
  it('should broadcast notifications on insert', done => {
    ...
    idb
      .insert('todos', [{ name: 'todo5' }, { name: 'todo6' }])
      .toArray()
      .subscribe(
        () => {},
        () => {},
  ...
  it('should iterate records with a predicate fn', done => {
    let found: any;
    idb
      .query('todos', rec => rec.name === 'todo5')
      .toArray()
      .subscribe(
        (records: any) => {
          found = records;
